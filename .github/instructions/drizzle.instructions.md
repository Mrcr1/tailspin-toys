...
- Add a TSDoc/JSDoc comment to every exported function in `db/` and `src/lib/`. The comment must describe the function's purpose, every parameter (including the injectable `db` argument), and the return value. Keep the explanation focused on intent and data contracts; do not paraphrase the implementation.
- Update or remove comments that no longer match the code whenever the related code changes.
